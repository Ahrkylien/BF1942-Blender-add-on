...
def bf42_placeObject(objectTemplate, sceneScale, base_path, level, bf42_data, position = None, rotation = None, loadFarLOD = False):
    ...
    if bf42_is_linked(objectTemplate):
        ...
        meshCollection = bf42_getCollectionByName(objectTemplate,MultiMeshObjectsCollection)
        instance_obj = bpy.data.objects.new(objectTemplate, None)
        instance_obj.instance_collection = meshCollection
        instance_obj.instance_type = 'COLLECTION'
        print("Error: "+objectTemplate+" objectTemplate does not exist!!")
    instance_obj.location = position.toBlend(sceneScale)
    bf42_applyRotation(instance_obj,rotation)
    instance_obj.name = objectTemplate.name if bf42_is_linked(objectTemplate) else objectTemplate
