...
def light_map_export(object_and_names, directory, sceneScale, option = "All - Diffuse", size = 128):
    bpy.context.scene.render.engine = 'CYCLES'

    # duplicate objects for bake and hide the oridginal ones:
    objects=[]
    objectsHiddenFromRender=[]
    for (object_src, name) in object_and_names:
        if len(object_src.material_slots.values()) > 0:
            if object_src.material_slots[0].material != None:
                if "lightMap" in object_src.data.uv_layers:
                    ...
                    objects.append((bf42_duplicateSpecialObject(object_src), image, name))
    ...
    for (object, image, name) in objects:
        ...
        filePath = directory+"\\"+name+"_"+pos.str_floor()+".tga"
